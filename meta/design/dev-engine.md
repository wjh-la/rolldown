...
└─────────┬─────────┘  └─────────┬─────────┘                  │
          ...
          │  err → FullBuildFailed│  err → Failed { stage }   │
          │  ok  → Idle ─────────┼──ok──→ Idle ───────────────┤
          ▼                      ▼  (then schedule_build_if_  │
┌───────────────────┐  ┌───────────────────┐    stale always)│
│  FullBuildFailed  │  │ Failed { stage }  │                  │
└─────────┬─────────┘  └─────────┬─────────┘                  │
          │ next file change:    │ next file change:          │
          │  queue FullBuild,    │  stage == Hmr → queue Hmr  │
          │  schedule →          │     (or HmrRebuild on      │
          │  FullBuildInProgress │     Always strategy);      │
          ▼                      │  stage == Rebuild → queue  │
       (loop)                    │     HmrRebuild;            │
                                 │  schedule →                │
                                 │  InProgress ───────────────┘
...
| `InProgress → Failed { last_error_stage }`                         | `handle_bundle_completed`, `:288`              |
